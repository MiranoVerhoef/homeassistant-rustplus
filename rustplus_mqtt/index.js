...
function loadState() {
  try {
    ...
    return { rust: { server: "", port: 0, steam_id: "", player_token: "" }, devices: { switches: [], alarms: [], cameras: [] } };
  }
}
...
function normalizeImported(obj) {
  const server =
    ...
    obj.ip ||
    obj.host ||
    (obj.serverDetails && obj.serverDetails.ip) ||
    "";

  const port = Number(
    obj.port ||
    obj.appPort ||
    (obj.serverDetails && obj.serverDetails.port) ||
    0
  );
...
console.log("Rust+ MQTT Bridge starting (v0.2.1)...");
console.log("Ingress UI listening on :8099");
console.log("MQTT:", `${options.mqtt.host}:${options.mqtt.port}`);
console.log("State file:", STATE_PATH);

// MQTT connect (for later discovery/state publishing)
...
app.get("/", (_req, res) => {
  ...
  res.send(page(`
    <div class="card">
      <h1>Rust+ Pairing Wizard <span class="pill">v0.2.1</span></h1>
      <p class="muted">Paste or upload Rust+ credentials produced by the standard Rust+ bot pairing flow. No admin access required.</p>
    ...
    <div class="card">
      <h2>Devices</h2>
      <p class="muted">Store your entity lists here for now (auto-discovery comes next).</p>
    ...
    <div class="card">
      <h2>Next</h2>
      <ul>
        <li>Connect to Rust+ using saved credentials</li>
        <li>Publish MQTT Discovery entities</li>
        <li>Poll states + accept switch commands</li>
      </ul>
    </div>
  `));
});
...
app.get("/health", (_req, res) => res.json({ ok: true, version: "0.2.1" }));
